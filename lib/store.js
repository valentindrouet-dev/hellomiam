...
export const TABLES = ['recipes', 'prices', 'cart_recipes', 'cart_extras', 'cart_checked', 'saved_carts']
...
function blankState() {
  return { recipes: [], prices: [], cart: { recipes: [], extras: [], checked: {} }, savedCarts: [] }
}
...
function readLocal() {
  try {
    ...
    // Les paniers mémorisés sont arrivés après : une sauvegarde plus ancienne
    // n'en a pas, et doit continuer de s'ouvrir sans broncher.
    if (raw?.recipes && raw?.prices && raw?.cart) return { ...raw, savedCarts: raw.savedCarts ?? [] }
...
export class Store {
  ...
  async load() {
    ...
    const [recipes, prices, cartRecipes, extras, checked, savedCarts] = await Promise.all([
      ...
      pg(this.config, 'GET', 'saved_carts', { query: '?select=*&order=saved_at.desc' }),
    ])
    this.state = {
      ...
      savedCarts: (savedCarts ?? []).map(rowToSavedCart),
    }
    return this.state
  }
  ...
  // Mise à jour d'une partie d'une recette (notes, tags, ajustements) sans
  // avoir à reconstruire l'objet entier.
  async patchRecipe(id, fields) {
    return this.updateRecipe(id, { ...this.state.recipes.find(r => r.id === id), ...fields })
  }
  ...
  // ——— Paniers mémorisés ———
  //
  // Un panier gardé, c'est la liste des recettes choisies un jour donné avec
  // leur nombre de convives : de quoi refaire les mêmes courses, ou se
  // rappeler ce qu'on avait mangé cette semaine-là.

  async saveCurrentCart(name) {
    const snapshot = {
      id: newId(),
      name: String(name || '').trim().slice(0, 80) || 'Panier',
      savedAt: nowIso(),
      recipes: this.state.cart.recipes.map(c => ({ recipeId: c.recipeId, adults: c.adults, children: c.children })),
      extras: this.state.cart.extras.map(e => ({ name: e.name, qty: e.qty, unit: e.unit, dept: e.dept })),
    }
    if (this.config) await pg(this.config, 'POST', 'saved_carts', { body: [savedCartToRow(snapshot)] })
    this.state.savedCarts.unshift(snapshot)
    if (!this.config) writeLocal(this.state)
    return snapshot
  }

  async deleteSavedCart(id) {
    if (this.config) await pg(this.config, 'DELETE', 'saved_carts', { query: `?id=eq.${id}` })
    this.state.savedCarts = this.state.savedCarts.filter(c => c.id !== id)
    if (!this.config) writeLocal(this.state)
  }

  async renameSavedCart(id, name) {
    const clean = String(name || '').trim().slice(0, 80)
    if (!clean) return
    if (this.config) await pg(this.config, 'PATCH', 'saved_carts', { query: `?id=eq.${id}`, body: { name: clean } })
    this.state.savedCarts = this.state.savedCarts.map(c => (c.id === id ? { ...c, name: clean } : c))
    if (!this.config) writeLocal(this.state)
  }

  // Remet le panier mémorisé à la place du panier courant. Les recettes
  // supprimées de la bible entre-temps sont ignorées, et le nombre de celles
  // qui manquent est renvoyé pour pouvoir le dire.
  async restoreCart(id) {
    const snapshot = this.state.savedCarts.find(c => c.id === id)
    if (!snapshot) return null
    const connues = snapshot.recipes.filter(r => this.state.recipes.some(x => x.id === r.recipeId))
    const perdues = snapshot.recipes.length - connues.length

    await this.clearCart()
    for (const r of connues) await this.setCartRecipe(r.recipeId, r.adults, r.children)
    for (const e of snapshot.extras) await this.addExtra(e)
    return { restaurees: connues.length, perdues }
  }
...
create table if not exists recipes (
  ...
  tags_off jsonb not null default '[]',
  swaps jsonb not null default '{}',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- Bases créées avant les sous-catégories, tags et ajustements : sans effet
-- sur une base neuve, ces colonnes y sont déjà.
alter table recipes add column if not exists sub text;
alter table recipes add column if not exists tags jsonb not null default '[]';
alter table recipes add column if not exists tags_off jsonb not null default '[]';
alter table recipes add column if not exists swaps jsonb not null default '{}';
