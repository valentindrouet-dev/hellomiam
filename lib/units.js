...
// Objets qui ne se coupent pas en quarts dans une cuisine.
const WHOLE_ONLY = new Set(['pincée', 'sachet', 'boîte', 'cube', 'feuille', 'brin'])

// Arrondi "cuisine" : on ne demande pas 187,33 g de riz mais 190 g, et surtout
// pas 1 ¾ poivron. Chaque famille a sa granularité utilisable.
export function smartRound(q, unit) {
  if (q == null || !Number.isFinite(q)) return q
  const info = unitInfo(unit)

  // Les cuillères doseuses existent en quarts : on peut être précis.
  if (info.spoon) return Math.max(0.25, Math.round(q * 4) / 4)

  if (info.family === 'count') {
    // Une pincée, une boîte, un cube : ça se compte en entiers.
    if (WHOLE_ONLY.has(info.unit)) return Math.max(1, Math.round(q))
    // Le reste (pièce, gousse, tranche…) se coupe en deux, pas en quatre.
